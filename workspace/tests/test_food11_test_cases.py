...
# Require 60% accuracy
def test_cake_looks_like_accuracy(model, predict):
    acc = evaluate_folder(model, CAKE_DIR, predict)
    assert acc >= 60, f"{CAKE_DIR} accuracy too low: {acc:.2f}%"

# Require 60% accuracy
def test_indian_dessert_accuracy(model, predict):
    acc = evaluate_folder(model, INDIAN_DIR, predict)
